...
plays = pd.read_csv('../historical_data/PBP.csv')
...
def gameIndices(dfObj, game):
    ...
    while not gameStart:
        while not gameFound:
            ...
            if index >= len(
                    dfObj):  #when i exceeds df length, cut iteration by 2
                i = i / 2
...
print(plays.loc[(plays['gid'] == 1) & (plays['bc'] == 'CG-0400'), 'yds'].sum())
print(plays.loc[(plays['gid'] == 1) & (plays['trg'] == 'CG-0400'), 'yds'].sum())
